...
const About = () => {
  const [jannickURL, setJannickURL] = useState(null);

  useEffect(() => {
    const storage = getStorage();

    const jannickRef = ref(storage, 'images/jannick.webp');
    getDownloadURL(jannickRef)
      .then((url) => {
        setJannickURL(url);
      })
      .catch((error) => {
        console.error('Error getting download URL for Jannick image:', error);
      });
  }, []);
    ...
    <div id='about' className='border-b border-purple pb-20 xl:pb-0 -mt-32 pt-32 '>
      <div className='max-w-[1500px] m-auto'>
        <div className='grid grid-cols-1 xl:grid-cols-3 gap-8 m-auto'>
          <img className='rounded-full col-span-1 md:grid-cols-1 p-8 w-[50%] xl:w-[100%] m-auto' src={jannickURL} alt="Jannick" />
